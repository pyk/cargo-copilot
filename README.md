...
| `crate_overview`     | Fetches the main documentation page for a specific crate. It runs `cargo doc --package <name> --no-deps` internally to ensure docs are available.       |
| `crate_symbol_list`  | Lists all symbols (modules, macros, structs, enums, functions, types) found in a crate's generated documentation.                                       |
| `crate_symbol_get`   | Retrieves the full documentation for a specific symbol (e.g., `de/struct.Deserializer`) as Markdown.                                                    |
...
2. Use `crate_overview` to understand the high-level purpose of a crate.
3. Use `crate_symbol_list` to explore the API surface of a crate.
4. Use `crate_symbol_get` to read specific documentation for functions, structs, or traits.
